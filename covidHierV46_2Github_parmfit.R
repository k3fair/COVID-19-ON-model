...
library(abind); library(plyr); 

#Read in all ncessary data for fitting
DATA=readRDS("covidHierData.rds"); #See readme file for description of contents
cmat=readRDS("cmat_data_weighted.rds"); #Contact matrix data from Prem et al. (2020)
regionid=readRDS("PHUtoREGIONlinker_numeric.rds"); #COntains data for linking census divisions to their associated PHU
...
inClen_w2=rep(46+workstaggergap1,49); #days between Dec 26th lockdown and Feb 10th first reopening day
inClen_s2=rep(49+schoolstaggergap,49); #days between Dec 21st first day of christmas holiday, feb 8th first reopening day
...
#Modifier of travel matrix as people in I and/or tested positive less likely to travel by a proportion pstay
...
m3iter=function(S,parms,timeinfo,seed,Ns=parms[,"N"]){
  ...
  #Make modified travel matrices for people in I and/or tested positive, then implement travel
...
closeinappl=function(parms,TS,tm=dim(TS)[3],delayInit=10){
  ...
  if( (max(Nta)>=inCstart) & (tm-which(Nta>=inCstart)[1])<(158-1) ) { parms[,"omg"]=omgs[,1];} #individual NPI adherence with risk perception coeff omg_0 before ramp down begins
  if((max(Nta)<inCstart)) parms[,"omg"]=0; #No individual NPI adherence before start of pandemic (as inCstart is March 10th when total cases>=50)
...
msim3=function(FUN,parms,Trun=365,seed0=11,plotgive=TRUE,InitInf=c(6:10)){
  ...
  ### 3 returns just (combined) closures, 3.5 returns succeptibles and (combined) closures (to use succeptibles for determining infections)
...
while (any(parmStr < lowerBex | parmStr > upperBex)) {parmStr=parmStr0*(1.1-0.2*runif(length(pfit)));} #print("Reset IC"); }
...
x=nloptr(parmsFit=fitnames[pfit],opts=list("algorithm"="NLOPT_GN_ISRES","xtol_rel"=1e-5,"maxeval"=2500,"print_level"=2), x0=parmStr[pfit],eval_f=LLfun,lb=lowerBex[pfit],ub=upperBex[pfit],reps=nrep,extras=extras);
y=nloptr(parmsFit=fitnames[pfit],opts=list("algorithm"="NLOPT_LN_COBYLA","xtol_rel"=1e-15,"maxeval"=500,"print_level"=2), x0=x$solution,eval_f=LLfun,lb=lowerBex[pfit],ub=upperBex[pfit],reps=nrep,extras=extras);

#Get time to create unique identifier for output from fitting algorith run
clock<-unclass(as.POSIXlt(Sys.time()))
# Save output from fitting algorithm run
saveRDS(x, file = sprintf("parmfit_x_%s_d%ih%im%ims%d.rds", codeversion, clock$mday, clock$hour, clock$min, round(1000*clock$sec)));
saveRDS(y, file = sprintf("parmfit_y_%s_d%ih%im%ims%d.rds", codeversion, clock$mday, clock$hour, clock$min, round(1000*clock$sec)));
