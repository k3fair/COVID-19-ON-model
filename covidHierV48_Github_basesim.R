...
library(abind); library(plyr); library(tidyverse);
...
for (fit in 1:length(file.names))
{
  ...
  if (y$objective<1055) #Throw out any parm combinations that don't give a reasonably good fit (reduces runtime by throwing out any parameter sets where the cost function value from the fitting is quite high)
  {
    print(y$objective)
...
saveRDS(df.region.comb, file = sprintf("simsREGIONS_%s.rds", codeselect));
saveRDS(df.prov.comb, file = sprintf("simsPROVINCE_%s.rds", codeselect));
saveRDS(df.fitparms.comb, file = sprintf("simsFITPARMS_%s.rds", codeselect));
...
saveRDS(fullparm.storage, file = sprintf("simsFULLFITPARMS_%s.rds", codeselect));
