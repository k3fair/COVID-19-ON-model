#     covidHierV48_Github_firstcounterfactual.R runs simulations for counterfactuals for 10 March 2020 to 15 August 2020 (presence/absence of closures and/or NPI adherence)
...
saveRDS(df.region.comb, file = sprintf("simsREGIONS_all_COUNTERFACTUAL_%s_%s_%s_%s.rds", codeversion, cftype, reopeningtype, vdtype));
saveRDS(df.prov.comb, file = sprintf("simsPROVINCE_all_COUNTERFACTUAL_%s_%s_%s_%s.rds", codeversion, cftype, reopeningtype, vdtype));
saveRDS(df.fitparms.comb, file = sprintf("simsFITPARMS_all_COUNTERFACTUAL_%s_%s_%s_%s.rds", codeversion,  cftype, reopeningtype, vdtype));
