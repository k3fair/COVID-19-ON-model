#     covidHierV48_Github_secondcounterfactual.R runs simulations for counterfactuals for 12 June 2020 to 28 February 2021 (presence/absence of NPIs in workplaces/schools when they reopen)
...
#Drop all dates where totals are iffy because of reporting lags
...
for (fit in 1:topnum) {
  ...
  lockdownstart= workclosuregap2 + 20;
  lockdownend=workclosuregap2+inClen_w2;
  ...
  #Dummy start values for closures
  inCstart_w=50;
  inCstart_s=50;
  ...
  m3iter=function(S,parms,timeinfo,seed,Ns=parms[,"N"]){
    ...
    if (timeinfo[4]>=lockdownstart) {
      S[, "VD"] <- ifelse(timeinfo[4]<lockdownend, (1-exp(-(parms[1,"omg"]*pos + parms[1,"boost"]))), (1-exp(-parms[1,"omg"]*pos))) #Stay at home, with staggered reopening
    }
  ...
  msim3=function(FUN,parms,Trun=365,seed0=11,plotgive=TRUE,InitInf=c(6:10)){
    ...
    Mn=round((parms[1,"Mfact"]/2)*parms[,-(1:NP)]*(1-diag(L))); diag(Mn)=parms[,"N"]-colSums(Mn); parms[,-(1:NP)]=Mn%*%diag(1/colSums(Mn));
    ...
    if(plotgive=="fit"){ 
      omgs=t(apply(TS[,"VD",], 2, function(x) meansd(x,parms[,"N"])));
...
for (rocoeffval in 1:length(rocoeff.list)) {

rocoeff<-rocoeff.list[rocoeffval]

parms[,c("reopen_w")] = base.reopen_w*rocoeff
parms[,c("reopen_s")] = base.reopen_s*rocoeff

if(rocoeff==10){parms[,c("reopen_w")] = base.reopen_w; parms[,c("reopen_s")] = base.reopen_s; inClen_s1=inClen_w1=schoolclosuregap2=workclosuregap2=1e3;} #Set these to 1e3 so never reopen

print(parms[1,c("reopen_s", "reopen_w")])

#Run multiple sims based on our best parm sets for different reopening efficacies
nrun<-5
runlength<-425
...
for (run in 1:nrun) {
  ...
  #Quick plot
  agedat.plot<-agedat[agedat$Date<="2021-02-21",]
  ...
  plot(agedat.plot$ts50, rowSums(agedat.plot[,c( "newK.1", "newK.2", "newK.3", "newK.4", "newK.5")]),ylim=range(c(dat$Nt-dplyr::lag(dat$Nt), rowSums(agedat.plot[,c( "newK.1", "newK.2", "newK.3", "newK.4", "newK.5")])),xlim=range(c(agedat.plot$ts50, dat$ts50)), na.rm=T))
...
saveRDS(df.region.comb, file = sprintf("simsREGIONS_all_CFW2REOPEN_%s_%s_%s.rds", codeversion, cftype, reopeningtype));
saveRDS(df.prov.comb, file = sprintf("simsPROVINCE_all_CFW2REOPEN_%s_%s_%s.rds", codeversion, cftype, reopeningtype));
saveRDS(df.fitparms.comb, file = sprintf("simsFITPARMS_all_CFW2REOPEN_%s_%s_%s.rds", codeversion,  cftype, reopeningtype));
