#     Script visualizes output from covidHierV46_2Github_secondcounterfactual.R
...
### DATA VIZ
library(tidyverse); library(scales); library(Hmisc); library(ggnewscale);
`%notin%` <- Negate(`%in%`);

#Select set of sims to visualize
code.list<-c("CFW2REOPEN_v46v2phuNUDGE_neither_restricted")
...
if (exists('mobilitydat')==FALSE) #Skips running this step if has already been done (marked by existence of final object generated in this step)
{
  ...
  df.dates<-data.frame(Date=(seq(as.Date("2020-01-01"), as.Date("2021-04-30"), by = "days")))
  df.50<-data.frame(Date=(seq(as.Date("2020-01-01"), as.Date("2021-04-30"), by = "days")), ts50=1:length(seq(as.Date("2020-01-01"), as.Date("2021-04-30"), by = "days")) - length(seq(as.Date("2020-01-01"),temp50$Date[min(which(temp50$totalcases>=50))], by="days")))
...
for(codeselect in 1:length(code.list))  {
  ...
  if(exists("p.comb.main")==FALSE) {codeversioncheck<-codeversion;} #If we haven't run code yet, this obj doesn't exist and we'll set the code version for the check
  
  if ((codeversioncheck!=codeversion | (codeversioncheck==codeversion & exists('simdeath')==FALSE))) #Skips running this data collating/hosp outcome generating step if we're looking at the same codeeversion and this step has already been done (marked by existence of final object generated in this step)
  {
    ...
    fitparms00=readRDS(sprintf("simsFITPARMS_all_%s_V2.rds", codeversion));
    simdata00=readRDS(sprintf("simsPROVINCE_all_%s_V2.rds", codeversion));
    ...
    rw.tots$week<-as.Date(cut(rw.tots$Date, breaks = "week")) 
    ...
    simdata$week<-as.Date(cut(simdata$Date, breaks = "week")) 
    simdeath <- simdata %>% group_by(run,fit,rocoeff,week) %>% dplyr::summarise(newcases.weekly=sum(new.K))
    simdeath <- simdeath %>% group_by(run,fit,rocoeff) %>% mutate(cases.l2=lag(newcases.weekly,2), weekly.deaths.worst=cfrfcn(cases.l2, week)*cases.l2) %>% ungroup()
  ...
  limselect<-c(as.Date("2020-06-12"), as.Date("2021-02-28")); #Set span of dates to plot 
  dbreak<-"1 month";
  rw.agg.scenario<-rw.agg[(rw.agg$Date >= limselect[1] & rw.agg$Date <= limselect[2]),]
  
  #Set transparency of lines from individual sims
  alphaval<-0.1; rw.line<-0.5; summary.line<-0.5;
  custom.pal<- c( "black", rev(brewer.pal(4,"RdBu")[c(1,3:4)]))
  
  simdata <- simdata %>% mutate(tag=factor(rocoeff, levels = c(-1, 10,1,0)))
  
  p.cases.1<-ggplot() +
    geom_line(data=simdata, aes(x=Date, y=new.K, group=interaction(run,fit, rocoeff), colour=tag), alpha=alphaval) +
    stat_summary(data=simdata, fun=median,geom="line",aes(x=Date, y=new.K, colour=tag), size=summary.line, key_glyph = "rect")+
    scale_colour_manual("Scenario", values=custom.pal[-1], labels=c("Remain closed" , "Reopen (NPIs in workplaces/schools)","Reopen (no NPIs in workplaces/schools)"), limits=c("10","1","0")  , guide = guide_legend(order = 1) ) +
    new_scale_color() +
    scale_colour_manual("", values=custom.pal[1], labels=c("Actual"), limits=c("-1")  , guide = guide_legend(order = 1) ) +
    geom_line(data=rw.tots, aes(x=Date, y=new.K, colour=factor(-1, levels = c(-1, 10,1,0))), key_glyph = "rect") +
    xlab("Date") +
  ...
  p.cases.2<-ggplot() +
    geom_line(data=simdata[simdata$rocoeff %in% c(1,10),], aes(x=Date, y=new.K, group=interaction(run,fit, rocoeff), colour=tag), alpha=alphaval) +
    stat_summary(data=simdata[simdata$rocoeff %in% c(1,10),], fun=median,geom="line",aes(x=Date, y=new.K, colour=tag), size=summary.line, key_glyph = "rect")+
    geom_line(data=rw.tots, aes(x=Date, y=new.K, colour=factor(-1, levels = c(-1, 10,1,0)))) +
    xlab("Date") +
    ylab("New confirmed cases") +
    scale_x_date(expand = c(0,0), limits=limselect, date_breaks = dbreak, date_labels = "%b '%y") +
    scale_y_continuous(labels=comma, expand = c(0.01,0)) +
    scale_colour_manual("Source", values=custom.pal, labels=c("Actual","Scenario: Remain closed" , "Scenario: Reopen (NPIs)","Scenario: Reopen (no NPIs)"), limits=c("-1","10","1","0")) +
    theme_light() + guides(colour="none") + theme(axis.text.x = element_text(angle = 45, hjust = 1),plot.margin = margin(0, 5, 2, 0, "pt"))
  
  p.cases<- p.cases.1 + inset_element(p.cases.2, 0.005, 0.49, 0.515, 0.995 , ignore_tag = TRUE)
  
  ## Bar charts
  simdeath.sum.0 <- simdeath[(simdeath$week>=limselect[1] & simdeath$week<=limselect[2]),] %>% group_by(run,fit,rocoeff) %>% dplyr::summarise(model.deathsweeklyK.best=sum(weekly.deaths.best, na.rm=TRUE),
                                                                                                                                              model.deathsweeklyK.worst=sum(weekly.deaths.worst, na.rm=TRUE))
  simdeath.sum.ci <- simdeath.sum.0 %>% group_by(rocoeff) %>% dplyr::summarize(
  ...
  simdeath.sum.1 <- simdeath.sum.0 %>% group_by(rocoeff) %>% dplyr::summarize(mean.model.deathsweeklyK.best=mean(model.deathsweeklyK.best),
  ...
  rw.weekly.sum$rocoeff<--1
  
  simdeath.sum.2<-rbind(simdeath.sum.1, rw.weekly.sum)
  simdeath.sum.2$groups <- factor(simdeath.sum.2$rocoeff,  levels = c(-1, 10,1,0))
  ...
  h.deathweekly.best<- ggplot() +
    ...
    scale_y_continuous(labels=comma, expand=c(0,0.01), limits=c(0,35000)) +
    scale_fill_manual("Source", values=custom.pal, labels=c("Actual","Scenario: Remain closed" , "Scenario: Reopen (NPIs)","Scenario: Reopen (no NPIs)"  ), drop=F) +
    theme_light() +
  ...
  p.comb.main<- (p.cases + guide_area() + plot_layout(guides="collect",widths=c(0.625,0.375))) / (h.deathweekly.best + guides(fill="none") + h.deathweekly.worst + guides(fill="none"))   + 
  ...
  hel=lo
  png("CFplot_wave2_main_V2.png", width=20, height=20, units="cm", res=500)
  print(p.comb.main)
  dev.off()
  
  #To check if we've switched versions before allowing shortcut
  codeversioncheck<-codeversion
